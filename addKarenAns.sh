...
answers=( "Shepherd Spinal Center does evaluations and assessments for those who use power chairs or who may need adapted vehicles. Check out: http://www.shepherd.org/patient-programs/outpatient-clinics/adapted-driving-services"
          ...
          "Yes, check out this info on free or discounted prescription medication programs in GA
          http://www.dcor.state.ga.us/pdf/Drug_Assistance_Programs.pdf"
...
# Add Emily (answer giver)
curl -i -H "Content-Type: application/json" -X POST -d '{"username":"Emily","email":"emily@email.com","password":"emilypass"}' http://localhost:5000/register
...
do
     ...
     do
        ...
        curl -u Emily:emilypass -i -H "Content-Type: application/json" -X PUT -d '{"answer": "'"$answer"'"}' $c
        COUNTER=$[COUNTER+1]
